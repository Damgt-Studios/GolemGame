...
void AD_UI::Text2D::Initialize(ID3D11Device1* _device, ID3D11DeviceContext1* _context)
{
...
void AD_UI::UILog::InitializeLog(float screenWidth, float screenHeight, float width, float height)
{
    blackBox.quad = new ADResource::AD_UI::QuadData{ 0.35f * screenWidth, -0.3f * screenHeight, 400, 250, 0.1046f, 0.2083f, 0.8645f, 0.9876f };
    blackBox.visible = true;
    consoleLabel = { true, XMFLOAT2(0.75f * screenWidth, 0.7f * screenHeight), {"0"} };
}

int AD_UI::UILog::ProcessInput()
{
    return 0;
}
...
int AD_UI::Button2D::ProcessInput()
{
...
AD_UI::ButtonList::ButtonList(float _x, float _y, float _spacing, UINT _columns, UINT _maxRows, bool _active, bool _visible, bool _controlFocus)
{
    x = _x;
    y = _y;
    spacing = _spacing;
...
int AD_UI::ButtonList::ProcessInput()
{
...
bool AD_UI::OverlayController::ProcessResponse(UINT responseID)
{ 
    return false;
}

void AD_UI::OverlayController::AddComponent(UINT _compID)
{ 
...
int AD_UI::ADUI::AddNewOverlay(bool _visible, bool _active, bool _dynamic)
{
...
AD_ULONG AD_UI::ADUI::AddUIComponent(ADResource::AD_UI::UIComponent* component)
{
...
void AD_UI::ADUI::AddUIController(OverlayController* _controller)
{
...
void AD_UI::ADUI::RefreshOverlay(ComPtr<ID3D11DeviceContext1> _context)
{
    for (int over = 0; over < setup->overlays.size(); ++over)
    {
        if (setup->overlays[over].IsDynamic() && setup->overlays[over].visible)
        {
            ...
            {
                ADResource::AD_UI::Overlay2D ovl = setup->overlays[over];
...
void AD_UI::ADUI::Initialize(ID3D11Device1* device, ID3D11DeviceContext1* _context)
{
    ...
    context = _context;
    text.Initialize(device, _context);
    ...
    for (int overly = 0; overly < setup->overlays.size(); ++overly)
    {
        if (visible && setup->overlays[overly].visible)
        {
            ...
            for (int i = 0; i < setup->overlays[overly].componentIDs.size(); ++i)
            {
                ...
                ADResource::AD_UI::Overlay2D ovl = setup->overlays[overly];
                UINT cid = ovl.componentIDs[i];
