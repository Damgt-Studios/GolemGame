			...
			else if (lhs == "Action")
			{
				...
				while (endPos != std::string::npos)
				{
					...
					else if (lhs == "HitboxDuplicates")
					{
						...
						for (UINT i = 1; i < action->hitboxCount; ++i)
						{
...
public:
	...
	bool LoadGameUserInterface(ADUI::ADUI* _userInterface, ADAI::PathingGrid* _grid)  //, AD_AUDIO::ADAudio* _audioEngine
	{
		gameUI.SetupUI(_userInterface, _grid);  //, _audioEngine
